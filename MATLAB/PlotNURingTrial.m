function fig = PlotNURingTrial(tt, varargin)
    ...
    %       'Animate'         (default false) - in addition to the static
    %                           figure, play the 3D trajectory and the three
    %                           stacked subplots point-by-point over time,
    %                           and export the animation as a video file
    %                           whose PLAYBACK duration matches the trial's
    %                           real-world duration (t_secs(end)-t_secs(1)).
    %                           Frame-accurate: the video is rendered at a
    %                           fixed frame rate (VideoFrameRate), but each
    %                           output frame shows whichever sample was
    %                           current at that real elapsed time -- so
    %                           irregular sample spacing (dropped frames,
    %                           variable loop timing) still displays with
    %                           the correct real-world delay between
    %                           visible updates, rather than every sample
    %                           getting equal screen time.
    %       'SourceFilename'  (default '')   - path or name of the source
    %                           trial CSV file. When Animate is true, the
    %                           output video is saved next to it using the
    %                           same base filename (e.g.
    %                           "123-...-303.csv" -> "123-...-303.mp4"). If
    %                           omitted, a generic filename is used in the
    %                           current folder and a warning is issued.
    %       'VideoFrameRate'  (default 30)   - fixed output frame rate (fps)
    %                           used when Animate is true. Higher values
    %                           give smoother motion but don't change the
    %                           overall video duration, which is always
    %                           locked to the trial's real-world duration.
    %
    %   Example:
...
function AnimateAndExportVideo(fig, t, tx, ty, tz, txm, tym, tzm, vxm, vym, vzm, ...
    ...
    %   velocity lines and points, draws growing "animated" copies plus a
    %   moving "current position" marker on top frame-by-frame, captures
    %   each frame, and restores the original static lines once done so the
    %   figure handed back to the caller still shows the complete static
    %   plot.

    numSamples = numel(t);
    duration   = t(end) - t(1);
    if numSamples < 2 || duration <= 0
        warning('PlotNURingTrial:AnimateAndExportVideo:badTimebase', ...
    ...
    if baseNameSource == ""
        warning('PlotNURingTrial:AnimateAndExportVideo:noSourceFilename', ...
    ...
    hAnimPtX   = scatter(axX, nan, nan, 50, 'k', 'filled', 'HandleVisibility', 'off');

    hold(axY, 'on');
    hAnimLineY = plot(axY, nan, nan, 'Color', Colors.greMd, 'LineWidth', Globals.lineWidthData);
    hAnimPtY   = scatter(axY, nan, nan, 50, 'k', 'filled', 'HandleVisibility', 'off');

    hold(axZ, 'on');
    hAnimLineZ = plot(axZ, nan, nan, 'Color', Colors.bluMd, 'LineWidth', Globals.lineWidthData);
    hAnimPtZ   = scatter(axZ, nan, nan, 50, 'k', 'filled', 'HandleVisibility', 'off');
    ...
    % still shows the complete, static plot.
    % ---------------------------------------------------------------
    ...
    fprintf('PlotNURingTrial: Animation saved to %s\n', outFile);
