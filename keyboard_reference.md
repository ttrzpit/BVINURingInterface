...
-----------|----------|--------------------------------------------|----------------------------|-------------------------|-------------------------------------------------------------------------------
 ...
 `w`       | 119      | "Scan the blank workspace (mask + pose lock)" | OBJ_SCAN, OBJ_SEL, OBJ_RUN | (same as previous)  | "Scanning world markers (keep the workspace clear)... N" then "Masked N world marker(s), pose LOCKED (M frames) - place the objects, then [t] to train." (see Note5, Note6)
 ...
 `r`       | 114      | "Randomly pick a TRAINED object from pool" | OBJ_SEL or OBJ_RUN         | OBJ_RUN                 | "Re-scanning objects..." then "Object marker set to [OBJECT_ID] (N present) - E-STOP released, timing." (see Note3, Note7)
...
**Note2** `u` forgets every trained anchor. Objects revert to live "(preview)" wireframes (drawn only while their marker is visible) and produce NO guidance target until re-trained. `u` does NOT clear the `w` world-marker mask.
**Note7** In OBJECTS, `r` and `SPACEBAR` bracket a retrieval trial. The moment `r` actually arms a target (i.e. after the presence re-scan resolves - not on the keypress, since the scan may find nothing to guide to) it also RELEASES the e-stop for full guidance output and starts the "Object timer" shown above the recording stamp. The `SPACEBAR` press that re-ENGAGES the e-stop marks the trial complete and stops that timer; the final time stays on screen (green instead of white) until the next `r` starts a new one. Releasing the e-stop by hand with `SPACEBAR` does not restart the timer - only `r` does. A manual `m` selection arms a target WITHOUT touching the e-stop or the timer.
...
**Note5** `w` masks the printed world board out of the operator view (and therefore out of the logged video). Press it with the workspace BLANK - no objects placed: a burst (`object_world.train_frames` detection frames, same length as `t`) accumulates the image corners of every detected world marker, and each marker seen on >= 3 burst frames becomes one averaged, slightly padded pixel quad that is filled solid WHITE from then on. The quads are FROZEN in pixel space, so no world pose is needed to draw them, they never flicker, and they stay put once objects and the reaching hand occlude the board - which is exactly why the scan must happen before the objects go down. The faint blue outlines on live-detected world markers are still drawn on top as a detection diagnostic. The mask is only valid while the camera does not move: after any camera change, clear the workspace and press `w` again (a second press discards the old mask and re-scans). `u` leaves it alone; leaving and re-entering OBJECTS (`O`) drops it. The status line carries the current count as "mask: N". Opacity is `object_world.world_mask_alpha` (default 1.0 = opaque): lower values wash the markers out without hiding them, which makes a camera bump obvious (the box stops lining up with its marker), but ArUco patterns stay legible below about 0.85, so use 1.0 for participant runs. 0 hides the mask without clearing it.
**Note6** The same `w` burst also LOCKS the world->camera pose (`object_world.lock_world_pose`, default on): the per-frame solved pose is averaged over the burst and frozen, and training, anchor rendering, guidance targets, contact detection and the ring ground-plane hit all then run off that one constant. This is what stops trained objects jumping when a hand occludes part of the board: the per-frame solve re-derives the pose from whatever marker subset is visible, and since `marker_positions` entries are only consistent to a few mm, each subset settles on a slightly different pose and shifts every object together. Locked, occlusion changes nothing and trained objects never coast (a pose is always available). The live solve keeps running as a diagnostic. Staleness: each frame the detected markers' mean corner reprojection error against the lock is measured, and sustained error past `object_world.world_lock_drift_px` (default 3 px, ~15 frames) warns "World pose lock has drifted - camera moved?" - clear the workspace and press `w` again. Anchors are stored in the WORLD frame, so re-locking after a camera move keeps every trained object. The status line reads `pose: LOCK 1.2px` while locked (`OK`/`--` when the lock is off). Set `lock_world_pose: 0` to A/B against the old per-frame behaviour.
...
-----------|---------------------------------------------------------|-------------------------
 ...
 `w`       | Scan blank workspace: mask markers + lock world pose    | OBJECTS
