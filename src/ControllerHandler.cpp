...
ControllerHandler::ControllerHandler( const ControllerConfig& cfg )
    : cfg_( cfg ), preload_A_( cfg.tension_preload_min ), preload_B_( cfg.tension_preload_min ), preload_C_( cfg.tension_preload_min ) {
...
void ControllerHandler::Update( const TeensyToPcPacket& rx,
    ...
    measuredForce_x_ = CONSTANT_UNIT_VECTOR_A_X * measuredTension_A + CONSTANT_UNIT_VECTOR_B_X * measuredTension_B + CONSTANT_UNIT_VECTOR_C_X * measuredTension_C;
    measuredForce_y_ = CONSTANT_UNIT_VECTOR_A_Y * measuredTension_A + CONSTANT_UNIT_VECTOR_B_Y * measuredTension_B + CONSTANT_UNIT_VECTOR_C_Y * measuredTension_C;
    ...
    // ---- Force ramp: scales the Stage 1 guidance force by rampValue_ (0..1) ---
    // ramp_type: 0 = none (full output immediately)
    //            1 = startup ramp - climbs 0->1 over ramp_duration_secs since the
    //                new target appeared (ResetRamp).
    //            2 = distance ramp - climbs 0->1 as the fingertip-to-target depth
    //                (displacement_.z) closes by initial_ramp_percentage of the
    //                depth captured at first detection. One-directional: it only
    //                ramps up, so retreating (depth increasing) never reverses it.
    switch ( cfg_.ramp_type ) {
        case 0:
            rampValue_ = 1.0f;
            break;
        ...
        case 2:
            ...
            if ( targetMarkerActive_ ) {
                ...
                if ( rampZInit_ ) {
                    const float closeSpan = rampStartZ_ * cfg_.initial_ramp_percentage;
                    if ( closeSpan > 1e-6f ) {
                        const float closed = rampStartZ_ - z;    // depth closed since start
                        float       r = std::clamp( closed / closeSpan, 0.0f, 1.0f );
                        rampValue_ = std::max( rampValue_, r );    // one-directional
                    } else {
                        rampValue_ = 1.0f;    // percentage 0 -> no distance gating
                    }
                }
            }
            break;
...
void ControllerHandler::ResetRamp( double nowSecs ) {
    rampStart_ = nowSecs;
    rampValue_ = 0.0f;
    rampStartZ_ = 0.0f;    // distance ramp: recapture start depth on first detection
    rampZInit_ = false;
...
float ControllerHandler::ComputeEffectiveRadius( float q_abs ) const {
    ...
    return CONSTANT_MOTOR_PULLEY_RADIUS + ( CONSTANT_TENDON_THICKNESS / CONSTANT_TWO_PI ) * std::max( q_abs, 0.0f );
}
...
float ControllerHandler::ComputeTendonLengthChange( float q_abs, float q_home ) const {
    ...
    auto F = [this]( float q ) {
        ...
        return CONSTANT_MOTOR_PULLEY_RADIUS * q + ( CONSTANT_TENDON_THICKNESS / CONSTANT_FOUR_PI ) * ( qPos * qPos );
    };
...
uint16_t ControllerHandler::CurrentToPwm( float current ) const {
    ...
    float pwm = static_cast<float>( CONSTANT_PWM_OFF ) + frac * static_cast<float>( CONSTANT_PWM_MAX - CONSTANT_PWM_OFF );
...
void ControllerHandler::SetManualTension( char motor, float valueN ) {
    ...
    switch ( motor ) {
        ...
        case 'C':
            ...
            break;
        case 'D':
            tension_A_ = tension_B_ = tension_C_ = clamped;
            break;
        default:
            break;
    }
