...
// ---- Random-target exclusion -------------------------------------------------
// IDs in cfg_.excludedTargetIds (set via config.yaml: fitts_board_assignments ->
// random_target_excluded_ids) are STILL rendered on the board and STILL detected
// for pose/guidance - they are only removed from the random ("r") target pool.
inline bool IsRandomTargetExcluded( const std::vector<int>& excluded, int id ) {
...
FittsBoardLayout::FittsBoardLayout( const FittsBoardConfig&  cfg,
    ...
    for ( int r = 0; r < cfg_.fineRows; r++ ) {
        for ( int c = 0; c < cfg_.fineCols; c++ ) {
            ...
            // unless explicitly excluded from the random pool (still rendered
            // and detected - see kRandomTargetExcludedIds at top of file).
            if ( r >= loRow && r <= hiRow && c >= loCol && c <= hiCol &&
                 !IsRandomTargetExcluded( cfg_.excludedTargetIds, id ) )
