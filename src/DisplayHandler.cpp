...
void DisplayHandler::PopulateControllerPanel(const std::vector<DetectedMarker> &markers,
    ...
    switch (kb.systemState) {
        case SystemState::CALIBRATING:    systemStateStr = "CALIBRATING";    break;
        case SystemState::CAL3:           systemStateStr = "CAL3";           break;
        case SystemState::FITTS:          systemStateStr = "FITTS";          break;
        case SystemState::PRETENSION:     systemStateStr = "PRETENSION";     break;
        case SystemState::TENSION_ADJUST: systemStateStr = "TENSION ADJUST"; break;
        default:                          systemStateStr = "IDLE";           break;
    }
    ...
    if (serial.hasRx) {
        switch (static_cast<char>(serial.lastRx.state)) {
            case 'W': teensyStateStr = "WAITING"; break;
            case 'I': teensyStateStr = "IDLE";    break;
            case 'D': teensyStateStr = "DRIVING"; break;
            case 'R': teensyStateStr = "READY";   break;
            default:  teensyStateStr = std::string(1, static_cast<char>(serial.lastRx.state)); break;
        }
    }
    ...
    AddControllerBodyCell("--", "E6", 4, 1, "center", bodyFontSize, Colors::GraBk);  // Amplifier state — not yet reported by Teensy
    ...
    AddControllerBodyCell(fmtMm(std::sqrt(controllerTele_.vel_virtual.x * controllerTele_.vel_virtual.x
                                         + controllerTele_.vel_virtual.y * controllerTele_.vel_virtual.y)), "L10", 2, 1, "center", bodyFontSize);
    ...
    AddControllerBodyCell(fmtMm(std::sqrt(controllerTele_.posErrorIntegral.x * controllerTele_.posErrorIntegral.x
                                         + controllerTele_.posErrorIntegral.y * controllerTele_.posErrorIntegral.y)), "L11", 2, 1, "center", bodyFontSize);
    ...
    AddControllerBodyCell("--", "G18", 3, 1, "center", tableFontSize);
    AddControllerBodyCell("--", "J18", 3, 1, "center", tableFontSize);
    AddControllerBodyCell("--", "M18", 3, 1, "center", tableFontSize);
    // Proportional Gain — direction-dependent 'G' boost added to kP_effective
    ...
    // Output — preload + guidance-force contribution, clamped to [tension_min, tension_max]
